...
public class DeadPlayer : MonoBehaviour
{
    ...
    void CheckPlayersDeath()
    {
        if (deadPlayers == 3)
        {
            ...
            //1生き残ったら
            if (DeadPlayerNumber[0] != 1 && DeadPlayerNumber[1] != 1 && DeadPlayerNumber[2] != 1)
            {
                score[0]++;
                if (score[0] > 3) score[0] = 1;
                Debug.Log("プレイヤー1の勝利");
            }
            else if (DeadPlayerNumber[0] != 2 && DeadPlayerNumber[1] != 2 && DeadPlayerNumber[2] != 2)
            {
                score[1]++;
                if (score[1] > 3) score[1] = 1;
                Debug.Log("プレイヤー2の勝利");
            }
            else if (DeadPlayerNumber[0] != 3 && DeadPlayerNumber[1] != 3 && DeadPlayerNumber[2] != 3)
            {
                score[2]++;
